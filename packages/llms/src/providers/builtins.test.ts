import { ZENUXS_ENVIRONMENT_ENV, ZENUXS_ENVIRONMENTS } from "@cline/shared";
...
describe("cline builtin spec defaults.baseUrl", () => {
	const originalEnvironment = process.env[ZENUXS_ENVIRONMENT_ENV];

	beforeEach(() => {
		delete process.env[ZENUXS_ENVIRONMENT_ENV];
	});
	...
	afterEach(() => {
		if (originalEnvironment === undefined) {
			...
			process.env[ZENUXS_ENVIRONMENT_ENV] = originalEnvironment;
		}
	});
	...
	it("re-resolves baseUrl when CLINE_ENVIRONMENT changes between reads", () => {
		...
		expect(spec.defaults?.baseUrl).toBe(
			`${ZENUXS_ENVIRONMENTS.production.apiBaseUrl}/api/v1`,
		);

		process.env[ZENUXS_ENVIRONMENT_ENV] = "staging";
		expect(spec.defaults?.baseUrl).toBe(
			`${ZENUXS_ENVIRONMENTS.staging.apiBaseUrl}/api/v1`,
		);

		process.env[ZENUXS_ENVIRONMENT_ENV] = "local";
		expect(spec.defaults?.baseUrl).toBe(
			`${ZENUXS_ENVIRONMENTS.local.apiBaseUrl}/api/v1`,
		);
...
describe("cline-pass builtin spec", () => {
	it("registers a distinct Cline-compatible provider with a custom model list", async () => {
		...
		expect(provider).toMatchObject({
			id: "cline-pass",
			name: "ClinePass",
			baseUrl: `${ZENUXS_ENVIRONMENTS.production.apiBaseUrl}/api/v1`,
