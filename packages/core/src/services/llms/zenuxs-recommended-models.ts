...
export interface ZenuxsRecommendedModel {
	id: string;
	name: string;
	description: string;
	tags: string[];
...
export interface ZenuxsRecommendedModelsData {
	recommended: ZenuxsRecommendedModel[];
	free: ZenuxsRecommendedModel[];
}

export interface FetchZenuxsRecommendedModelsOptions {
	baseUrl?: string;
...
export const FALLBACK_ZENUXS_RECOMMENDED_MODELS: ZenuxsRecommendedModelsData = {
	recommended: [
		{
...
function cloneRecommendedModels(
	data: ZenuxsRecommendedModelsData,
): ZenuxsRecommendedModelsData {
	return {
...
function normalizeModel(raw: unknown): ZenuxsRecommendedModel | null {
...
function normalizeResponse(raw: unknown): ZenuxsRecommendedModelsData | null {
	if (!raw || typeof raw !== "object") return null;
	const data = raw as Record<string, unknown>;
	...
	const recommended = recommendedRaw
		.map(normalizeModel)
		.filter((model): model is ZenuxsRecommendedModel => model !== null);
	const free = freeRaw
		.map(normalizeModel)
		.filter((model): model is ZenuxsRecommendedModel => model !== null);
...
function getConfiguredApiBaseUrl(
	options: FetchZenuxsRecommendedModelsOptions,
): string {
	const explicitBaseUrl = options.baseUrl?.trim();
	if (explicitBaseUrl) return explicitBaseUrl;

	const fallbackBaseUrl = getZenuxsEnvironmentConfig().apiBaseUrl;
	try {
		const manager =
...
export async function fetchZenuxsRecommendedModels(
	options: FetchZenuxsRecommendedModelsOptions = {},
): Promise<ZenuxsRecommendedModelsData> {
	try {
	...
	return cloneRecommendedModels(FALLBACK_ZENUXS_RECOMMENDED_MODELS);
}
