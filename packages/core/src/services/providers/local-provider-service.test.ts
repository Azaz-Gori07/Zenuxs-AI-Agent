...
describe("deleteLocalProvider", () => {
	...
	it("does not unregister built-in provider when deleted/disconnected", async () => {
		manager.saveProviderSettings(
			{
				provider: "nvidia",
				baseUrl: "https://example.invalid/v1",
		...
		expect(manager.getProviderSettings("nvidia")).toBeDefined();

		await deleteLocalProvider(manager, {
			providerId: "nvidia",
		});

		expect(manager.getProviderSettings("nvidia")).toBeUndefined();
		expect(LlmsModels.hasProvider("nvidia")).toBe(true);
	});
