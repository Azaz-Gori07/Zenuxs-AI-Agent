...
import type { ZenuxsCoreStartInput } from "./engine/types";
import type {
	StartSessionInput,
	StartSessionResult,
...
import { ZenuxsCore } from "./ZenuxsCore";
import { NoOpFeatureFlagsProvider } from "./services/feature-flags";

function createStartInput(): ZenuxsCoreStartInput {
	return {
		config: {
...
describe("ZenuxsCore", () => {
	...
	it("applies start-session bootstraps before delegating to the host", async () => {
		...
		const applyToStartSessionInput = vi.fn(
			async (input: ZenuxsCoreStartInput) => ({
				...input,
				config: {
					...input.config,
	...
	it("prefers the per-session telemetry service over the ZenuxsCore one", async () => {
		const host = {
			runtimeAddress: undefined,
	...
	it("can list sessions without hydrating message history", async () => {
		...
		// ZenuxsCore/listSessionHistory; the host list contract only receives the
		// numeric scan limit.
	...
	it("exposes event automation through ZenuxsCore instead of CronService", async () => {
		const root = mkdtempSync(join(tmpdir(), "zenuxs-core-automation-"));
