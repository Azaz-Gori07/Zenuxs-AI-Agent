...
import {
	ZenuxsCoreAutomationController,
	createZenuxsCoreAutomationExtensionContext,
	createZenuxsCoreAutomationRuntimeHandlers,
	normalizeAutomationCronScope,
	normalizeAutomationOptions,
} from "./engine/automation";
import {
	createZenuxsCorePendingPromptsApi,
	createZenuxsCoreSettingsApi,
	type RuntimeHostServiceExtensions,
} from "./engine/runtime-services";
import {
	normalizeZenuxsCoreStartInput,
	toZenuxsCoreStartInput,
} from "./engine/start-input";
import { emitSessionStartedTelemetry } from "./engine/telemetry";
import type {
...
/**
 * The primary entry point for the Zenuxs Core SDK.
 *
 * @example
 * ```ts
 * import { ZenuxsCore } from "@cline/core";
 *
 * const cline = await ZenuxsCore.create({ clientName: "my-app" });
 * const session = await cline.start({ ... });
 * ```
 */
export class ZenuxsCore {
	readonly clientName: string | undefined;
	readonly runtimeAddress: string | undefined;
	readonly automation: ZenuxsCoreAutomationApi;
	readonly settings: ZenuxsCoreSettingsApi;
	readonly featureFlags: FeatureFlagsService;
	readonly pendingPrompts: PendingPromptsServiceApi;
	private readonly host: RuntimeHost;
	private readonly prepare: ZenuxsCoreOptions["prepare"] | undefined;
	...
	private constructor(
		...
		prepare: ZenuxsCoreOptions["prepare"],
		...
		automationOptions:
			| (ZenuxsCoreAutomationOptions & { logger?: BasicLogger })
			| undefined,
	) {
		...
		this.settings = createZenuxsCoreSettingsApi(host);
		this.pendingPrompts = createZenuxsCorePendingPromptsApi(host);
		this.automation = new ZenuxsCoreAutomationController(() => {
			if (!this.automationService) {
				throw new Error(
					"ZenuxsCore automation is not enabled. Pass `automation: true` or automation options to ZenuxsCore.create().",
				);
			}
		...
		this.automationService = automationOptions
			? new CronService({
					...
					runtimeHandlers: createZenuxsCoreAutomationRuntimeHandlers({
						host,
						getExtensionContext: () =>
							createZenuxsCoreAutomationExtensionContext({
	...
	/**
	 * Creates a new ZenuxsCore instance.
	 *
	 ...
	 * @returns A promise that resolves to a new ZenuxsCore instance
	 *
	 * @example
	 * ```ts
	 * const cline = await ZenuxsCore.create({
	 *   clientName: "my-app",
	 *   backendMode: "local",
	 * });
	 * ```
	 */
	static async create(options: ZenuxsCoreOptions = {}): Promise<ZenuxsCore> {
	 ...
	 */
	start(input: ZenuxsCoreStartInput): Promise<StartSessionResult>;
	async start(
		input: StartSessionInput | ZenuxsCoreStartInput,
	): Promise<StartSessionResult> {
		const zenuxsCoreInput = toZenuxsCoreStartInput(input);
		const bootstrap = await this.prepare?.(zenuxsCoreInput);
		try {
			const preparedInput = bootstrap
				? await bootstrap.applyToStartSessionInput(zenuxsCoreInput)
				: zenuxsCoreInput;
			const result = await this.host.startSession(
				normalizeZenuxsCoreStartInput(preparedInput, {
	...
	/**
	 * Disposes the ZenuxsCore instance and all associated resources.
	 *
	 ...
	 */
	listHistory = async (
		options: ZenuxsCoreListHistoryOptions = {},
	...
	list = async (
		limit = 200,
		options: Omit<ZenuxsCoreListHistoryOptions, "limit"> = {},
	): Promise<SessionHistoryRecord[]> =>
	...
	async restore(input: RestoreInput): Promise<RestoreResult> {
		const normalizedStart = input.start
			? normalizeZenuxsCoreStartInput(input.start, {
					defaultCapabilities: this.capabilities,
					withExtensionContext: (context) =>
						createZenuxsCoreAutomationExtensionContext({
							automationService: this.automationService,
