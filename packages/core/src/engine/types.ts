...
export type { ZenuxsCoreSettingsApi } from "../settings";
...
export interface ZenuxsCoreAutomationOptions {
...
export type ZenuxsAutomationSpec = CronSpecRecord;
export type ZenuxsAutomationRun = CronRunRecord;
export type ZenuxsAutomationEventLog = CronEventLogRecord;
export type ZenuxsAutomationEventSuppression = CronEventSuppression;
export type ZenuxsAutomationRunStatus =
	| "queued"
	| "running"
	| "done"
	| "failed"
...
export interface ZenuxsAutomationListSpecsOptions {
...
export interface ZenuxsAutomationListRunsOptions {
	specId?: string;
	status?: ZenuxsAutomationRunStatus | ZenuxsAutomationRunStatus[];
	limit?: number;
}

export interface ZenuxsAutomationListEventsOptions {
	eventType?: string;
	source?: string;
	processingStatus?:
		| "received"
...
export interface ZenuxsAutomationEventIngressResult {
	event: ZenuxsAutomationEventLog;
	duplicate: boolean;
	matchedSpecIds: string[];
	queuedRuns: ZenuxsAutomationRun[];
	suppressions: ZenuxsAutomationEventSuppression[];
}

export interface ZenuxsCoreAutomationApi {
	start(): Promise<void>;
	stop(): Promise<void>;
	...
	listEvents(
		options?: ZenuxsAutomationListEventsOptions,
	): ZenuxsAutomationEventLog[];
	getEvent(eventId: string): ZenuxsAutomationEventLog | undefined;
	listSpecs(options?: ZenuxsAutomationListSpecsOptions): ZenuxsAutomationSpec[];
	listRuns(options?: ZenuxsAutomationListRunsOptions): ZenuxsAutomationRun[];
}

export type ZenuxsCoreListHistoryOptions = SessionHistoryListOptions;

export interface ZenuxsCoreStartInput
...
export interface ZenuxsCoreOptions {
	/**
	...
	/**
	 * Feature flags service for this ZenuxsCore instance.
	...
	/**
	 * Enables file-based and event-driven automation through this ZenuxsCore
	 * instance. When configured, callers use `zenuxs.automation.*` instead of
	 * constructing cron services directly.
	 */
	automation?: boolean | ZenuxsCoreAutomationOptions;
	/**
...
export interface StartSessionBootstrap {
	applyToStartSessionInput(
		input: ZenuxsCoreStartInput,
	): Promise<ZenuxsCoreStartInput> | ZenuxsCoreStartInput;
