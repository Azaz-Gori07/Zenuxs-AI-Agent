...
export const CLINE_MCP_SETTINGS_FILE_NAME = "zenuxs_mcp_settings.json";
...
export function resolveClineDir(): string {
	...
	const envDir = process.env.CLINE_DIR?.trim() || process.env.ZENUXS_DIR?.trim();
	if (envDir) {
		return envDir;
	}
	return join(HOME_DIR, ".zenuxs");
}

export function resolveDocumentsClineDirectoryPath(): string {
	return join(HOME_DIR, "Documents", "Zenuxs");
}
...
export function resolveClineDataDir(): string {
	const explicitDir = process.env.CLINE_DATA_DIR?.trim() || process.env.ZENUXS_DATA_DIR?.trim();
	if (explicitDir) {
		return explicitDir;
	}
...
export function resolveSessionDataDir(): string {
	const explicitDir = process.env.CLINE_SESSION_DATA_DIR?.trim() || process.env.ZENUXS_SESSION_DATA_DIR?.trim();
	if (explicitDir) {
		return explicitDir;
	}
...
export function resolveTeamDataDir(): string {
	const explicitDir = process.env.CLINE_TEAM_DATA_DIR?.trim() || process.env.ZENUXS_TEAM_DATA_DIR?.trim();
	if (explicitDir) {
		return explicitDir;
	}
...
export function resolveDbDataDir(): string {
	const explicitDir = process.env.CLINE_DB_DATA_DIR?.trim() || process.env.ZENUXS_DB_DATA_DIR?.trim();
	if (explicitDir) {
		return explicitDir;
	}
...
export function resolveProviderSettingsPath(): string {
	const explicitPath = process.env.CLINE_PROVIDER_SETTINGS_PATH?.trim() || process.env.ZENUXS_PROVIDER_SETTINGS_PATH?.trim();
	if (explicitPath) {
		return explicitPath;
	}
...
export function resolveGlobalSettingsPath(): string {
	const explicitPath = process.env.CLINE_GLOBAL_SETTINGS_PATH?.trim() || process.env.ZENUXS_GLOBAL_SETTINGS_PATH?.trim();
	if (explicitPath) {
		return explicitPath;
	}
...
export function resolveMcpSettingsPath(): string {
	const explicitPath = process.env.CLINE_MCP_SETTINGS_PATH?.trim() || process.env.ZENUXS_MCP_SETTINGS_PATH?.trim();
	if (explicitPath) {
		return explicitPath;
	}
