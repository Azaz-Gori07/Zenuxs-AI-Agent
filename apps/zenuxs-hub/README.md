...
- start a new session from an initial prompt â€” workspace/provider/model are reused from the most recent session, or `CLINE_PROVIDER` / `CLINE_MODEL` env vars
...
The dashboard registers two clients with the hub: a `zenuxs-hub-server` (via `ClineCore`) for driving sessions and a `zenuxs-hub-server` (via `HubUIClient`) for the admin view.
