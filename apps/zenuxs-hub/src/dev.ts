...
function spawn(
	...
	void child.exited.then((code) => {
		if (!shuttingDown) {
			console.error(`[zenuxs-hub:dev] ${name} exited with code ${code}`);
			shutdown(code === 0 ? 0 : 1);
		}
	});
	return child;
...
console.log(`[zenuxs-hub:dev] Vite webview: ${webviewDevServerUrl}`);
console.log("[zenuxs-hub:dev] Hub dashboard: http://127.0.0.1:8787/");
