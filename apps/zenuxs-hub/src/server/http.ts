...
const THEME_BOOTSTRAP_SCRIPT = `<script id="zenuxs-hub-theme-bootstrap">
  (() => {
    try {
      const theme = window.localStorage.getItem("zenuxs-hub-theme");
...
export function normalizeWebviewIndexHtml(html: string): string {
	...
	if (normalized.includes('id="zenuxs-hub-theme-bootstrap"')) {
		return normalized;
	}
...
export class WebviewAssets {
	...
	private async serveIndex(): Promise<Response> {
		...
		return createTextResponse(
			"Zenuxs Hub webview is not built. Run `bun run build:webview` from apps/zenuxs-hub.",
			503,
		);
	}
