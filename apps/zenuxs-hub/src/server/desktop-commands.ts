import {
	addLocalProvider,
	type ZenuxsAccountActionRequest,
...
async function resolveHubZenuxsAccountAuthToken(input: {
...
export async function handleDesktopCommand(
	...
	if (command === "cline_account") {
		...
		const authToken = await resolveHubZenuxsAccountAuthToken({
			settings,
			apiBaseUrl,
		});
		if (!authToken) {
		...
		const accountService = new ZenuxsAccountService({
			apiBaseUrl,
			getAuthToken: async () => authToken,
		});
		return await executeZenuxsAccountAction(
			args as ZenuxsAccountActionRequest,
			accountService,
		);
	}
