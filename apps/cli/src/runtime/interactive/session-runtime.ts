...
export function createInteractiveSessionRuntime(input: {
	...
	// Keep a mutable config reference so model/provider changes can be applied
	// to future turns without destroying the active session.
	const configRef: { current: Config } = { current: input.config };

	function updateConfig(next: Config): void {
