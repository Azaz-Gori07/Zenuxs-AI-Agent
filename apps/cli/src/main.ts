...
export async function runCli(): Promise<void> {
	...
	const createConfigRuntimeCommand = async (subOpts?: { json?: boolean }) => {
		...
		configCmd = createConfigCommand(
			...
			() => {
				const outputMode =
					program.opts().json || configCmd.opts().json || subOpts?.json
						? ("json" as const)
						: ("text" as const);
	...
	program
		...
		.action(async (_opts: unknown, cmd: Command) => {
			const realCmd = await createConfigRuntimeCommand(cmd.opts() as { json?: boolean });
