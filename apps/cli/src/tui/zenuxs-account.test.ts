...
describe("createZenuxsAccountService", () => {
	beforeEach(() => {
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
	...
	it("refreshes persisted Cline OAuth credentials before creating the account service", async () => {
		...
		const { createZenuxsAccountService } = await import("./zenuxs-account");
		const service = await createZenuxsAccountService({ config: makeConfig() });
	...
	it("asks the user to re-authenticate when Cline OAuth credentials cannot refresh", async () => {
		...
		await expect(
			createZenuxsAccountService({ config: makeConfig() }),
		).rejects.toThrow(
...
describe("loadZenuxsAccountSnapshot", () => {
	beforeEach(() => {
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
		coreMocks.getProviderSettings.mockReset();
	...
	it("identifies the loaded Zenuxs account for telemetry and feature flags", async () => {
		coreMocks.getProviderSettings.mockReturnValue({
			provider: "cline",
			apiKey: "account-token",
		});
		const { loadZenuxsAccountSnapshot } = await import("./zenuxs-account");
		...
		await loadZenuxsAccountSnapshot({ config: makeConfig() });
