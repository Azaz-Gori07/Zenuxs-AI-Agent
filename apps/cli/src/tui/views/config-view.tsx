...
export function ConfigPanelContent(props: ConfigPanelProps) {
	...
	const clampedNavPosRef = useRef(clampedNavPos);
	clampedNavPosRef.current = clampedNavPos;
	const navIndicesRef = useRef(navIndices);
	navIndicesRef.current = navIndices;
	const handleToggleSelectedRef = useRef(handleToggleSelected);
	handleToggleSelectedRef.current = handleToggleSelected;
	...
	useDialogKeyboard((key) => {
		...
		const pos = clampedNavPosRef.current;
		const indices = navIndicesRef.current;
		if (key.name === "up") {
			const nextNavPos =
				pos > 0 ? pos - 1 : indices.length - 1;
			setNavPosition(nextNavPos);
			return;
		}
		if (key.name === "down") {
			const nextNavPos =
				pos < indices.length - 1 ? pos + 1 : 0;
			setNavPosition(nextNavPos);
			return;
		}
		if (key.name === "space") {
			handleToggleSelectedRef.current();
			return;
		}
		if (
			key.name === "d" &&
