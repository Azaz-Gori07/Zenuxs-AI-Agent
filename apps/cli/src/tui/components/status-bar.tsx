import { type AgentMode, Llms } from "@cline/core";
...
export function resolveModelDisplayName(config: {
	...
	if (config.thinking && config.reasoningEffort) {
		displayName = `${name} (${config.reasoningEffort})`;
	}
	if (config.providerId) {
		const collection = Llms.getProviderCollectionSync(config.providerId);
		const providerName = collection?.provider?.name;
		if (providerName) {
			displayName = `[${providerName}] ${displayName}`;
		}
	}
	return displayName;
}
