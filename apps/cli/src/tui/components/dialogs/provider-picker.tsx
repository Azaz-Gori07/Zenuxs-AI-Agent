...
export function ProviderPickerContent(
	...
	useDialogKeyboard((key) => {
		...
		if (key.name === "return") {
			const provider = filteredRef.current[safeSelectedRef.current];
...
export function DisconnectActiveProviderConfirmContent(
	props: ChoiceContext<boolean> & {
		providerName: string;
	},
) {
	const { resolve, dialogId, providerName } = props;
	const options = [
		{ value: true, label: "Disconnect and choose another provider" },
	...
	useDialogKeyboard((key) => {
		...
		if (key.name === "return" || key.name === "enter") {
			resolve(options[selectedRef.current].value);
			return;
		}
		if (key.name === "up" || (key.ctrl && key.name === "p")) {
			setSelected((s) => (s <= 0 ? options.length - 1 : s - 1));
	...
	return (
		<box flexDirection="column" gap={1}>
			<text fg="yellow">
				<strong>WARNING: You are disconnecting the currently active provider ({providerName}).</strong>
			</text>
			...
			<text fg="gray">↑/↓ navigate, Enter to select, Esc to cancel</text>
		</box>
	);
}
...
export function ProviderConfigInputContent(
	...
	useDialogKeyboard((key) => {
		...
		if (key.name === "tab" && fieldKeysRef.current.length > 1) {
			const idx = fieldKeysRef.current.indexOf(focusedFieldRef.current);
			const nextIdx = key.shift
				? (idx - 1 + fieldKeysRef.current.length) % fieldKeysRef.current.length
				: (idx + 1) % fieldKeysRef.current.length;
			const next = fieldKeysRef.current[nextIdx];
...
export function CodexCliStatusContent(
	...
	useDialogKeyboard((key) => {
		...
		if (key.name === "return" && statusRef.current?.installed) {
			resolve(true);
		}
	}, dialogId);
...
export function ProviderManagerContent(
	...
	useEffect(() => {
		if (providers.length > 0) {
			const idx = providers.findIndex((p) => p.id === currentProviderId);
			if (idx >= 0) setSelected(idx);
		}
	}, [providers, currentProviderId]);

	const optionCount = providers.length;
	const safeSelected = Math.min(selected, Math.max(0, optionCount - 1));

	const handleEdit = useCallback(async (id: string) => {
		await onEdit(id);
		loadProviders();
	}, [onEdit, loadProviders]);

	const handleDisconnect = useCallback(async (id: string) => {
		await onDisconnect(id);
		loadProviders();
	}, [onDisconnect, loadProviders]);
	...
	useDialogKeyboard((key) => {
		...
		const provider = providersRef.current[safeSelectedRef.current];
		if (!provider) return;
		...
		if (key.name === "up") {
			setSelected((s) => {
				...
				return count === 0 ? 0 : s <= 0 ? count - 1 : s - 1;
			});
			return;
		}
		if (key.name === "down") {
			setSelected((s) => {
				const count = optionCountRef.current;
				return count === 0 ? 0 : s >= count - 1 ? 0 : s + 1;
			});
			return;
		}
		if (key.name === "e") {
			handleEditRef.current(provider.id);
			return;
		}
		if (key.name === "d") {
			handleDisconnectRef.current(provider.id);
			return;
		}
		if (key.name === "t") {
			handleTestRef.current(provider.id);
			return;
		}
	}, dialogId);
	...
	return (
		<box flexDirection="column" gap={1}>
			<text fg="cyan"><strong>Provider Manager</strong></text>

			{loading ? (
				<text fg="gray">Loading providers...</text>
			) : providers.length === 0 ? (
				<text fg="gray">No providers found</text>
			) : (
				<box flexDirection="column">
					{showAbove && (
						<box paddingX={1} justifyContent="center">
					...
					{visibleRows.map((row) => {
						...
						const p = providers[row.itemIndex];
						if (!p) return null;
						const isSel = row.itemIndex === safeSelected;
						const isCurrent = p.id === currentProviderId;
						const authed = p.isConfigured;
						return (
							...
							>
								...
								<text fg={isSel ? palette.textOnSelection : undefined} flexGrow={1}>
									{p.name}
								</text>
								{p.isOAuth && (
									<text
			...
			<text fg="gray">
				↑/↓ navigate | Enter set active | E configure | D disconnect | T test | Esc back
			</text>
		</box>
	);
}
