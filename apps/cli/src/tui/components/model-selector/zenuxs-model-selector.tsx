...
import type { ZenuxsModelPickerEntry } from "./zenuxs-model-picker";

export const BROWSE_ALL_ACTION = "__browse_all__";

type ZenuxsModelEntriesState =
	| { status: "loading"; message: string }
	| { status: "loaded"; entries: ZenuxsModelPickerEntry[] }
...
export function ZenuxsModelSelectorContent(
	props: ChoiceContext<string> & {
		currentModel: string;
		currentProviderName: string;
		knownModels?: Record<string, unknown>;
		entries: ZenuxsModelPickerEntry[];
	},
) {
	const {
		resolve,
	...
	return (
		<box flexDirection="column" gap={1}>
			...
			<box flexDirection="row" gap={1}>
				...
				<text fg="cyan">{currentProviderName}</text>
			</box>
			...
			<box flexDirection="column">
				{displayRows.map((row, idx) => {
					...
					const isSel = row.entryIndex === selected;
...
export function ZenuxsModelSelectorDialogContent(
	props: ChoiceContext<string> & {
		currentModel: string;
		currentProviderName: string;
		knownModels?: Record<string, unknown>;
		loadEntries: () => Promise<ZenuxsModelPickerEntry[]>;
	},
) {
	const { dismiss, dialogId, loadEntries } = props;
	const [state, setState] = useState<ZenuxsModelEntriesState>({
		status: "loading",
	...
	if (state.status === "loaded") {
		return <ZenuxsModelSelectorContent {...props} entries={state.entries} />;
	}
	...
	if (state.status === "error") {
		return (
			<box flexDirection="column" gap={1}>
				...
				<box flexDirection="row" gap={1}>
					<text fg="gray">Provider:</text>
					<text fg="white">{props.currentProviderName}</text>
				</box>
