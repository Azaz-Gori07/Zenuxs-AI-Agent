...
export function useRootKeyboard(input: {
	...
	onOpenConnectSelector?: () => void;
	onOpenModelsSelector?: () => void;
}) {
	...
	useKeyboard((key) => {
		...
		if (key.ctrl && key.shift && (key.name === "p" || key.name === "P")) {
			if (input.onOpenConnectSelector) {
				input.onOpenConnectSelector();
				return;
			}
		}

		if (key.ctrl && key.shift && (key.name === "m" || key.name === "M")) {
			if (input.onOpenModelsSelector) {
				input.onOpenModelsSelector();
				return;
			}
		}
