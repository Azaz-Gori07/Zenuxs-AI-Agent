import { getCurrentContextSize, summarizeUsageFromMessages, ProviderSettingsManager } from "@cline/core";
...
import { useConnectSelector } from "./hooks/use-connect-selector";
...
function App(props: TuiProps) {
	...
	const handleProviderChange = useCallback(async (providerId: string) => {
		props.config.providerId = providerId;
	...
	}, [props.config, handleModelChange]);

	const openConnectSelector = useConnectSelector({
		dialog,
		config: props.config,
		termHeight,
		refocusTextarea: () => refocusTextareaRef.current(),
	...
	const { handleSlashCommand } = useLocalCommandActions({
		...
		openConfig,
		openMcpManager,
		openModelsSelector: openModelSelector,
		openConnectSelector,
		openSkills,
		refocusTextarea: () => refocusTextareaRef.current(),
		setAppView,
	...
	useRootKeyboard({
		...
		onOpenConnectSelector: openConnectSelector,
		onOpenModelsSelector: openModelSelector,
	});
